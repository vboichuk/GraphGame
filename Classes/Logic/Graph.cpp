...
void Dijkstra::step(const Vertex* vertex)
{
	...
	for (auto [id, v] : neighbors)
	{
		...
		if (!util::contains(m_processed, id))
		{
			m_queue.insert(id);
		}
	}
...
bool GraphConfig::fromFile(const char* path)
{
	clear();
	...
	{
		bool ok = true;
		ok &= readValue(file, chips_num);
		ok &= readValue(file, vertex_num);
		...
		ok &= readVector(file, m_chips);
		ok &= readVector(file, m_chips_order);

		size_t edges_num = 0;
		ok &= readValue(file, edges_num);
...
bool GraphConfig::readValue(std::ifstream& file, OUT size_t& val)
{
	std::string line;
...
bool GraphConfig::readValues(std::ifstream& file, OUT float& val2, OUT float& val3)
{
	std::string line;
	if (getline(file, line))
	{
...
bool GraphConfig::readValues(std::ifstream& file, OUT tID& val2, OUT tID& val3)
{
	std::string line;
	if (getline(file, line))
	{
...
bool GraphConfig::readVector(std::ifstream& file, OUT std::vector<tID>& vector)
{
	std::string line;
	if (!getline(file, line))
		return false;

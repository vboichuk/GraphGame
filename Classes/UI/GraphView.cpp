...
static cocos2d::Color3B hueTorgb(int hue)
{
	if (hue > 360.0 || hue < 0)
	...
	return cocos2d::Color3B(r * 255, g * 255, b * 255);
}
...
bool GraphView::initChips(const std::vector<tID>& order)
{
	...
	auto menu = Menu::create();
	menu->setPosition(Vec2::ZERO);
	this->addChild(menu, 1, "Chips");

	const auto& chips = order; // m_graph->getChips();
